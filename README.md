...
**$\color{Green}{\text{Deep Improvement Supervision (DIS)}}$**. While the standard Tiny Recursive Model  relies on a **black-box** recurrence where the model must implicitly discover how to improve, DIS explicitly enforces a positive Advantage Margin at every recursive step. We achieve this by constructing a sequence of intermediate targets that strictly contract toward the solution. Our results are grounded in analyses that formally frame the latent reasoning of TRMs as both an $\color{red}{\text{implicit policy improvement}}$  algorithm and a form of $\color{Orange}{\text{classifier-free diffusion guidance}}$.
